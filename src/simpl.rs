use super::{algebra_tools::*, rational::*, scal::*};
...
const RULES: [fn(&Scalar) -> Vec<Scalar>; 3] = [
    ...
    |x| match x.as_ref() {
        ScalarInner::Sum(sum) => {
            ...
            fn index_factor(
                factors_indices: &mut FactorIndexer,
                factor_base: &Scalar,
                factor_exp: Rational,
                term_index: usize,
                factor_index: usize,
            ) {
                let index = FactorIndex {
                    t_idx: term_index,
                    f_idx: factor_index,
                    exp: factor_exp,
                };
                match factors_indices.get_mut(factor_base) {
                    ...
                    None => {
                        factors_indices.insert(factor_base.clone(), vec![index]);
                    }
                }
            }
                ...
                for i in 0..(indices.len() - 1) {
                    fn remove_factor(term: Scalar, factor_index: usize, exp: Rational) -> Scalar {
                        match term.as_ref() {
                            ScalarInner::Product(prod) => {
                                let to_remove =
                                    Scalar::from(prod.ref_factors()[factor_index].clone()) ^ exp;
                                term / to_remove
                            }
                            _ => Scalar::ONE,
                        }
                        ...
                        &indices[(i + 1)..]
                    {
                        if (exp1.numerator() < 0) == (exp2.numerator() < 0) {
                            let exp: Rational = if exp1.abs() < exp2.abs() { exp1 } else { exp2 };
                            let mut new_terms = sum.clone();
                            let term1 = new_terms.remove_term(t_idx1).unwrap();
                            let term2 = new_terms
                                .remove_term(if t_idx2 < t_idx1 { t_idx2 } else { t_idx2 - 1 })
                                .unwrap();

                            factorizations.push(
                                Scalar::from(new_terms)
                                    + (factor.clone() ^ exp)
            ...
            for index1 in 0..factors.len() {
                for index2 in 0..factors.len() {
                    ...
                    if let ScalarInner::Sum(_) = factors[index2].ref_base().as_ref() {
                        fn distribute(
                            distributions: &mut Vec<Scalar>,
                            other_factors: Scalar,
                            terms: Sum,
                            terms_exp: Rational,
                            distributed: Scalar,
                        ) {
                            ...
                            && index1 != index2
                        {
                            ...
                            let distributed =
                                other_factors.remove_factor(index1).unwrap().into_base();
                            let (terms_exp, terms) = match other_factors
                                .remove_factor(if index2 < index1 { index2 } else { index2 - 1 })
                                .unwrap()
                            {
                                factor => (*factor.ref_exponent(), Sum::from(factor.into_base())),
                            };

                            distribute(
                                &mut distributions,
                                Scalar::from(other_factors),
                                terms,
                                terms_exp,
                                distributed,
                            );
    ...
    |x| match x.as_ref() {
        ScalarInner::Product(prod) => Vec::new(),
        _ => Vec::new(),
    },
];
...
impl ExprCost {
    ...
    fn new(ops_count: usize, is_constant: bool) -> ExprCost {
        if is_constant {
            ExprCost { c: ops_count, v: 0 }
        } else {
            ExprCost { v: ops_count, c: 0 }
        }
    }
}
...
impl Scalar {
    fn cost(&self) -> ExprCost {
        match self.as_ref() {
            ScalarInner::Sum(s) => {
                merge_seq(
                    s.ref_terms().iter().map(|term| term.cost()),
                    |a, b| a + b,
                    ExprCost::ZERO,
                ) + ExprCost::new(s.ref_terms().len() - 1, self.is_constant())
            }
            ScalarInner::Product(p) => {
                merge_seq(
                    p.ref_factors()
                        .iter()
    ...
    fn complexity(&self) -> ExprComplexity {
        match self.as_ref() {
            ScalarInner::Sum(s) => merge_seq(
                s.ref_terms().iter().map(|term| term.complexity()),
                |a, b| a + b,
                ExprComplexity::ZERO,
            ),
            ScalarInner::Product(p) => merge_seq(
                p.ref_factors().iter().map(|factor| {
                    exponentiate(
                        factor.ref_base().complexity(),
...
impl Simplifier {
    ...
    fn derive(expr: &Scalar) -> Vec<Scalar> {
        //println!("{expr}");
        let mut transformed: Vec<Scalar> = RULES
            .iter()
            ...
            ScalarInner::Product(factors) => {
                for (i, factor) in factors.ref_factors().iter().enumerate() {
                    for trans_factor in Simplifier::derive(factor.ref_base()) {
                        ...
                        transformed.push(
                            Scalar::from(new_factors) * (trans_factor ^ *factor.ref_exponent()),
                        );
                    }
                }
            }
